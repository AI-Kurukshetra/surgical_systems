...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase.from("operating_rooms").select("*").eq("id", params.id).single();
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase.from("operating_rooms").update(payload).eq("id", params.id).select("*").single();
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  const { error } = await supabase.from("operating_rooms").delete().eq("id", params.id);
  if (error) return errorResponse(error.message, 400);
